...
Если Vite откажется работать в непустой директории — согласись на «Ignore files and continue»: в репозитории уже лежат `docs/` и `.gitignore`, их трогать не нужно.
...
`PracticeFlow` появится в Task 8, `AdminScreen` — в Task 10. До тех пор создай заглушки, чтобы сборка не падала:

```bash
mkdir -p src/ui/practice src/ui/admin
printf 'export function PracticeFlow() {\n  return <div className="app">Скоро</div>;\n}\n' > src/ui/practice/PracticeFlow.tsx
...
Проверь: страница открывается, видна кнопка «Начать»; переход по ссылке «Настройки» меняет адрес на `#/admin` и показывает заглушку админки; кнопка «назад» в браузере возвращает на стартовый экран.
...
- [ ] **Step 3: Подготовить репозиторий на GitHub**

Эти шаги выполняются один раз. Замени `<пользователь>` на своё имя на GitHub.
...
Затем в настройках репозитория: **Settings → Pages → Build and deployment → Source: GitHub Actions**.

Без этого переключателя workflow упадёт на шаге `configure-pages` с ошибкой
...
- [ ] **Step 5: Коммит и проверка деплоя**
...
Дождись завершения workflow (`gh run watch`) и открой
`https://<пользователь>.github.io/mathy/`. Проверь на телефоне или планшете:
пройди сессию целиком, убедись, что экранная клавиатура работает, результат
показывается, а после перезагрузки страницы история сохраняется.
