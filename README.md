...
Сессия запускается в одном из двух режимов, и режим сохраняется вместе с
результатом.

- **Тренировка** — правильность видна сразу после каждого примера. Похвала за
  верный ответ уезжает сама, разбор ошибки ждёт нажатия «Понятно».
- **Экзамен** — примеры идут подряд молча, всё видно только в итогах.

## Код от настроек

По умолчанию кода нет, и ребёнок может свободно открыть настройки. Чтобы
поставить код, задайте его в поле «Код от настроек» на экране админки. Код
хранится в открытом виде в `localStorage` под ключом `mathy.settings` — это
барьер от соблазна заглянуть, а не защита. Если код забыт, его можно
посмотреть или сбросить прямо там же через инструменты разработчика браузера.
